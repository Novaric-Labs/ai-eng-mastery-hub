...
const COURSE = 'ai-eng'; // the course these rows belong to (multi-course scoping)
...
out += `-- Seeds public.content for course '${COURSE}'. Safe to re-run (upsert on course_id,id).\n`;
out += '-- Requires migration 0003 (course_id column + courses anchor row).\n\n';
for (const r of rows) {
  out += `insert into public.content (course_id, id, tier, data) values ('${COURSE}', '${r.id}', '${r.tier}', ${sqlVal(r.data)})\n`;
